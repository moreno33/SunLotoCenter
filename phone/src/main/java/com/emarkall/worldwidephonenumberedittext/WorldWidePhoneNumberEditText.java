 ...
 */
public class WorldWidePhoneNumberEditText extends LinearLayout implements Validable {
    ...
    public boolean isValid(){
        try {
            if(phoneNumberUtil.isValidNumber(phoneNumberUtil.parse(country.getDialCode()+edxNumber.getUnMaskedText(),
                ...
                findViewById(R.id.errLayout).setVisibility(GONE);
                return true;
            }

            findViewById(R.id.errLayout).setVisibility(VISIBLE);
            return false;
        } catch (NumberParseException e) {
            return false;
        }
